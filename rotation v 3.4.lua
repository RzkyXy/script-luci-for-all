...
worldList = "C:\\Users\\Administrator\\Desktop\\LUCIFER\\FARM.txt" -- list farm for rotation
...
items = {
    ...
    {name = "Diamond Lock", id = 1796, emote = "<:DiamondLock:912341723919814657>"},
    ...
    {name = "Laser Grid", id = 5666, emote = "<:lgrid:980474405064941568>"},
    {name = "Laser Grid Seed", id = 5667, emote = "<:seed:980472844708352030>"},
...
bot.reconnect_interval = math.random(30,40)
...
function warp(world,id)
    ...
    if nuked then
    nukeWorldInfo(webhookNuked,world.." IS "..warpStatus.." @everyone")
    end
...
function displayList()
    ...
    for index, world in pairs(storageSeedList) do
        str = str .. "\n" .. index .. ". " .. (world:upper() or '?') .. " : "..(tableList[world] or "0").." "..itemInfo(itmSeed).emote
    end
    return str
end

function packInfo(link,id,desc,infoSeed)
    local text = [[
...
function reconnect(world,id,x,y)
    if restTimes then
        ...
        if isTrue >= 30 or math.floor(avoidTemp % 86400 % 3600 / 60) >= math.random(30,32) then
...
function cekSeed()
    ...
    for _, jum in pairs(bot:getWorld():getObjects()) do
...
function updateTableList()
    for worldSeed, _ in pairs(tableList) do
        tableList[worldSeed] = cekSeed()
    end
end

-- Update specific worldSeed if needed
function updateSpecificWorldSeed(worldSeed)
    if tableList[worldSeed] ~= nil then
        tableList[worldSeed] = cekSeed()
    end
end
...
function storeSeed(world)
    ...
    updateTableList()
    updateSpecificWorldSeed(storageSeed) 
    sleep(100)
    ba = ba - bot:getInventory():findItem(itmSeed)
    profitSeed = profitSeed + ba
    sleep(100)
    packInfo(webhookLinkSeed,messageIdSeed,infoPack(),true)
    sleep(100)
...
function pnb(world)
    if bot:isInWorld() then
        ...
        if changeColorSkin then
           bot.auto_skin.enabled = true
           bot.auto_skin.interval = 0.2
           bot.auto_skin.enabled = false
        end
    end
            ...
            if tile.fg == patokanStorage or tile.bg == patokanStorage then
                if includesNumber(IDItem,item) then
                    if tileDrop(tile.x,tile.y,bot:getInventory():findItem(item)) then
                        ...
                        if bot:getInventory():findItem(item) > 0 and tileDrop(tile.x,tile.y,bot:getInventory():findItem(item)) then
...
packInfo(webhookLinkPack,messageIdPack,infoPack(),false)
sleep(100)
bot.auto_collect = true
bot.collect_interval = 100
sleep(100)
